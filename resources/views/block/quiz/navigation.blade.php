...
<nav class="quiz">
	...
	<ol @if($isediting) class="sortable"  data-field="quizsort-{{$block->id}}" data-group="quiz-{{$block->id}}" data-url="/module/{{ $module->id }}/sortnodes/{{$block->node->id}}" @endif>
		...
		@if($block->node->children)
			...
			<li data-id="{{$child_node->id}}" class="@if($child_node->is($block->currentNode())) current @endif @if(method_exists($child_node->block, 'attemptForUser') && $child_node->block->attemptForUser()) answered @endif">
				@if($block->allow_navigation)
